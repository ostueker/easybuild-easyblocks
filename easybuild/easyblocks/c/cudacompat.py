...
class EB_CUDAcompat(Binary):
    ...
    def install_step(self):
        ...
        if LooseVersion(self.version) >= '11.8':
            library_globs.append('libcudadebugger.so.*')
        if LooseVersion(self.version) >= '13.0':
            library_globs.extend(['libnvidia-nvvm70.so.*', 'libnvidia-gpucomp.so.*',
                                  'libnvidia-pkcs11-*.so.*'])
        ...
        for library_glob in library_globs:
            ...
            if library.endswith('.' + nv_version):
                ...
                if 'nvvm.so' in library_glob:
                    versioned_symlink = library[:-len(nv_version)] + '4'
                if library_glob.startswith('libcuda') or 'ptx' in library_glob or 'nvvm.so' in library_glob:
    ...
    def sanity_check_step(self):
        ...
        if LooseVersion(self.version) >= '11.5':
            libraries.append('libnvidia-nvvm.so.4')
        custom_paths = {
